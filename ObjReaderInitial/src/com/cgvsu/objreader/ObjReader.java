...
public class ObjReader {
	...
	public static void deleteVertices(Model model, ArrayList<Integer> vertexIndicesToDelete) {
		// Удаляем полигоны, которые используют удаляемые вершины
		model.polygons.removeIf(polygon -> polygon.getVertexIndices()
				.stream()
				.anyMatch(vertexIndicesToDelete::contains));

		// Обновляем индексы вершин
		model.vertices = updateVertices(model.vertices, vertexIndicesToDelete);

		// Корректируем индексы вершин в полигонах
		for (Polygon polygon : model.polygons) {
			ArrayList<Integer> updatedIndices = new ArrayList<>();
			for (int vertexIndex : polygon.getVertexIndices()) {
				updatedIndices.add(correctIndex(vertexIndex, vertexIndicesToDelete));
			}
			polygon.setVertexIndices(updatedIndices);
		}
	}

	private static ArrayList<Vector3f> updateVertices(ArrayList<Vector3f> vertices, ArrayList<Integer> indicesToDelete) {
	...
	private static int correctIndex(int index, ArrayList<Integer> indicesToDelete) {
		int correctedIndex = index;
		for (int deletedIndex : indicesToDelete) {
